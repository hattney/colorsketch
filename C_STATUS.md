...
> ### ✅ Turnstile — 해결 (09-09). 원인은 Cloudflare 설정이 아니었다
>
> 09-04에 `400020`으로 위젯이 초기화에 실패해 "Cloudflare 위젯 설정 문제"로 적어 뒀으나
> **그 진단은 틀렸다.** Cloudflare 대시보드를 실제로 열어 보니 호스트 이름
> (`colorsketch-amber.vercel.app`)·위젯 모드(관리형)·사이트키가 전부 정확했다.
>
> **진짜 원인은 Turnstile이 자동화 브라우저를 정상적으로 차단한 것이었다.**
> 위젯 분석 화면에 챌린지 34건 발행 / **원본 브라우저 "Electron" / 해결률 0%**로 찍혀 있었다.
> 챌린지가 발행됐다는 것 자체가 사이트키·도메인 쌍이 이미 승인됐다는 뜻이고, 단지 봇으로
> 판정돼 토큰이 안 나온 것이다. 테스트 키로 바꿨을 때 통과한 이유도 같다 — 그 키는 봇 탐지를
> 건너뛴다.
>
> 09-09에 진짜 키 2개를 복원하고 검증했다:
> - 번들에 진짜 사이트키 `0x4AAAAAAEsYisnlNEZhlGFL` 실림 확인
> - `POST /api/ai-preview`에 **고의로 틀린 토큰** → **403** "Could not confirm you are a person"
>   → 테스트 시크릿이면 통과했을 것이므로 **진짜 시크릿이 들어가 있음이 증명됨**
>
> **⚠️ 앞으로 자동화 브라우저로는 Turnstile 통과 테스트를 할 수 없다.** 설계대로다.
> AI 프리뷰 생성을 자동으로 재검증해야 하면 Cloudflare 공개 테스트 키
> (`1x00000000000000000000AA` / `1x0000000000000000000000000000000AA`)로 잠시 바꿨다가
> **반드시 되돌릴 것.** 사람이 하는 확인은 일반 브라우저에서.
...
- **Vercel의 `Secret` 타입은 write-only다.** 저장 후 Edit을 열면 Value 칸이 비어 있고
  회색 플레이스홀더(`sk_live_a12…`)만 보여서 "저장이 안 됐다"로 오해하기 쉽다. 실제 판단
  근거는 행의 `Updated …` 타임스탬프, 그리고 Config 옵션이 "Saved secrets are write-only"
  사유로 비활성화돼 있는지다. 값이 맞는지는 **기능으로만** 확인된다.
...
| 우선 | 항목 | 내용 |
|---|---|---|
| ~~P0~~ ✅ | ~~용지 규격~~ **완료 09-09** | `src/utils/paper.ts` 신설 — 시트 치수의 단일 출처. 기본 **US Letter**, A4 선택 가능, 방향도 사용자 선택. 무료(클라 export)와 유료(서버 `upscaleToPaper`) 양쪽 반영, 주문에 시트를 기록해 발급 파일이 프리뷰와 일치. 검증: Letter 2550×3300 / A4 2480×3508 실측. 문구에서 "A4" 단독 표기 제거, 300 DPI 강조 완화 |
