...
- ~~US Letter 용지 지원~~ ✅ **완료 (09-09).** `src/utils/paper.ts`가 시트 치수의 단일
  출처다. 트레이스 비율·mm 선 굵기·내보내기 PNG·인쇄 스타일시트·서버 HD 업스케일이 전부
  여기서 파생되므로 서로 어긋날 수 없다. 기본값 **US Letter**, A4는 한 번의 클릭.
  방향(세로/가로)도 사용자가 고른다 — 이전엔 사진이 가로면 무조건 가로 시트였다.
  **구매자가 고른 시트는 프리뷰 시점에 주문에 기록**되고 `deliverOrder`가 그 위로
  업스케일하므로, 받는 파일이 화면에서 본 그 페이지다
